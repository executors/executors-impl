...
struct impl : impl_base
{
  ...
  impl_base* require_helper(property_list<Head, Tail...>, const type_info& t, const void* p,
    typename std::enable_if<Head::is_requirable>::type* = 0) const
  {
    if (t == typeid(Head))
    {
  ...
  impl_base* prefer_helper(property_list<Head, Tail...>, const type_info& t, const void* p,
    typename std::enable_if<Head::is_preferable>::type* = 0) const
  {
    if (t == typeid(Head))
    {
...
public:
  ...
  {
    auto e2 = std::require_concept(std::move(e), bulk_oneway);
  ...
  template<class Property,
    class = typename std::enable_if<
      bulk_oneway_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::is_requirable>::type>
