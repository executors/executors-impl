#ifndef STD_EXPERIMENTAL_BITS_THEN_EXECUTOR_H
#define STD_EXPERIMENTAL_BITS_THEN_EXECUTOR_H
...
#if 0
struct then_func_base
{
  virtual ~then_func_base() {}
  virtual any_ptr call(future<any_ptr>) = 0;
};

template<class Function, class R, class Arg>
struct then_func : then_func_base
{
  Function function_;

  explicit then_func(Function f) : function_(std::move(f)) {}

  virtual any_ptr call(future<any_ptr> value)
  {
    std::unique_ptr<then_func> fp(this);
...
using then_func_base = single_use_func_base<any_ptr, future<any_ptr>>;
template<class Function> using then_func = single_use_func<Function, any_ptr, future<any_ptr>>;
...
struct impl_base
{
  ...
  virtual future<any_ptr> then_execute(std::unique_ptr<then_func_base> f, future<any_ptr> fut) = 0;
...
struct impl : impl_base
{
  ...
  virtual future<any_ptr> then_execute(std::unique_ptr<then_func_base> f, future<any_ptr> fut)
  {
    return executor_.then_execute([f = std::move(f)](auto pred) mutable { return f.release()->call(std::move(pred)); }, std::move(fut));
  }
...
template<class... SupportableProperties>
class then_t::polymorphic_executor_type
{
public:
  ...
  template<class Executor> polymorphic_executor_type(Executor e,
      typename std::enable_if<then_executor_impl::is_valid_target_v<
        typename std::enable_if<!std::is_same<Executor, polymorphic_executor_type>::value, Executor>::type,
          SupportableProperties...>>::type* = 0)
  {
    auto e2 = execution::require(std::move(e), then);
    impl_ = new then_executor_impl::impl<decltype(e2), SupportableProperties...>(std::move(e2));
  }

  template<class... OtherSupportableProperties>
  polymorphic_executor_type(polymorphic_executor_type<OtherSupportableProperties...> e,
      typename std::enable_if<then_executor_impl::contains_exact_property_list_v<
        then_executor_impl::property_list<SupportableProperties...>,
  ...
  template<class Property,
    class = typename std::enable_if<
      then_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::is_requirable
  ...
  {
    then_executor_impl::find_convertible_property_t<Property, SupportableProperties...> p1(p);
  ...
  template<class Property,
    class = typename std::enable_if<
      then_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::is_preferable>::type>
  ...
  auto query(const Property& p) const
    -> typename std::enable_if<
      then_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::is_requirable
        || then_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::is_preferable,
      typename then_executor_impl::find_convertible_property_t<Property, SupportableProperties...>::polymorphic_query_result_type>::type
  {
    then_executor_impl::find_convertible_property_t<Property, SupportableProperties...> p1(p);
  ...
  auto then_execute(Function f, future<T> fut) const -> future<decltype(f(std::move(fut)))>
  {
    if (!impl_) throw bad_executor();

    using result_type = decltype(f(std::move(fut)));

    auto fut_wrap = fut.then(
        [](future<T> pred)
        {
          if constexpr (std::is_same<T, void>::value)
          {
            pred.get();
            return then_executor_impl::any_ptr();
          }
          else
          {
            return then_executor_impl::any_ptr(new T(pred.get()));
          }
        });

    auto f_wrap = [f = std::move(f)](future<then_executor_impl::any_ptr> pred)
    {
      if constexpr (std::is_same<result_type, void>::value)
      {
        f(pred.then(
              [](future<then_executor_impl::any_ptr> pred1)
              {
                if constexpr (std::is_same<T, void>::value)
                {
                  pred1.get();
    ...
    std::unique_ptr<then_executor_impl::then_func_base> fp(new then_executor_impl::then_func<decltype(f_wrap)>(std::move(f_wrap)));
    return impl_->then_execute(std::move(fp), std::move(fut_wrap)).then(
        [](future<then_executor_impl::any_ptr> pred)
        {
          if constexpr (std::is_same<result_type, void>::value)
          {
            pred.get();
  ...
  auto then_execute(Function f, future<T> fut) const -> future<decltype(f(std::move(fut)))>
  {
    ...
    std::unique_ptr<then_executor_impl::then_func_base> fp(new then_executor_impl::then_func<decltype(f_wrap)>(std::move(f_wrap)));
    impl_ ? impl_->then_execute(std::move(fp), std::move(fut_wrap)) : throw bad_executor();
  }
#endif
...
private:
  template<class...> friend class then_t::polymorphic_executor_type;
  polymorphic_executor_type(then_executor_impl::impl_base* i) noexcept : impl_(i) {}
  then_executor_impl::impl_base* impl_;
  const then_executor_impl::impl_base* get_impl() const { return impl_; }
};
...
#endif // STD_EXPERIMENTAL_BITS_THEN_EXECUTOR_H
